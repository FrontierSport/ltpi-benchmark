...
json_img_template = {"id": 0, "file_name": "", "width": 0, "height": 0}
...
def generate_crops_from_detections(
    det_path, crops_destination_dir, legible_results, images_dir
):
    all_legible = []
...
def generate_crops_tracklab(all_poses):
    saved_crops = []
    misses = 0
    for i, entry in enumerate(all_poses):
        filtered_points = get_points(entry)
        batch_id = entry["batch_id"]

        if len(filtered_points) == 0:
            # TODO: better approach then skipping
            # print(f"skipping, unreliable points")
            misses += 1
            continue
        img = entry["image"]
        if img is None:
        ...
        h, w, _ = crop.shape
        if h == 0 or w == 0:
            # print(f"skipping, shape is wrong")
            misses += 1
            continue
        saved_crops.append({"batch_id": batch_id, "img": crop})
    # print(f"skipped {misses} out of {len(all_poses)}")
    return saved_crops
...
def find_best_prediction(results, useBias=False):
    ...
    best_prediction = (
        unique_predictions[index_of_best] if best_weight > SUM_THRESHOLD else -1
    )
...
def get_calibrated_value(prob):
    ...
    if i == -1:
        ...
        return linear_interpolation(
            pred_conf[i], true_conf[i], pred_conf[i + 1], true_conf[i + 1], prob
        )
...
def apply_bias(raw_result):
    marginal_likelihood = raw_result[1][0] * 0.39 + sum(
        [x * 0.061 for x in raw_result[1][1:]]
    )
...
def initialize_priors(useBias, num_digits=11):
    ...
    if not useBias:
        return np.full(num_digits, 1.0 / num_digits), np.full(
            num_digits, 1.0 / num_digits
        )
    else:
...
def process_jersey_id_predictions_bayesian(
    file_path, useTS=False, useBias=False, useTh=False
):
    all_results = {}
    final_results = {}
...
def process_jersey_id_predictions(file_path, useBias=False):
    ...
    for tracklet in all_results.keys():
        ...
        best_prediction, all_unique, weights = find_best_prediction(
            results, useBias=useBias
        )
...
def evaluate_legibility(
    gt_path, illegible_path, legible_tracklets, soccer_ball_list=None
):
...
def evaluate_results(consolidated_dict, gt_dict, full_results=None):
    ...
    for id in gt_dict.keys():
        ...
        total += 1
    print(
        f"Total number of trackslets: {total}, correct: {correct}, accuracy: {100.0 * correct/total}%"
    )
...
def generate_crops_for_split(source, target, split):
    ...
    sample_illegible = random.sample(
        set(old_gt[old_gt["label"] == 0].image), number_legibles
    )
    all_images = []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "target", help="target directory for updated legibility dataset"
    )
    parser.add_argument(
        "--crops_based",
        action="store_true",
        default=False,
        help="extract crops for legibility dataset",
    )
    parser.add_argument(
        "--hockey",
        action="store_true",
