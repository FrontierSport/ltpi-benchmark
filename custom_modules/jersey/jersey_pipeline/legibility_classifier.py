...
def run_tracklab(inputs, model_ft, transform=None, threshold=0.5):
    if transform:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--new_trained_model_path", help="path to save newly trained model"
    )
    parser.add_argument(
        "--arch",
        choices=["resnet18", "simple", "resnet34", "vit"],
        default="resnet18",
    ...
    use_full_validation = (not args.full_val_dir is None) and (
        len(args.full_val_dir) > 0
    )
    ...
    dataloader_train = torch.utils.data.DataLoader(
        image_dataset_train, batch_size=4, shuffle=True, num_workers=4
    )

    if not args.train and not args.finetune:
        dataloader_test = torch.utils.data.DataLoader(
            image_dataset_test, batch_size=4, shuffle=False, num_workers=4
        )
    ...
    if use_full_validation:
        ...
        dataloader_full_val = torch.utils.data.DataLoader(
            image_dataset_full_val, batch_size=4, shuffle=False, num_workers=4
        )
        ...
        dataloader_val = torch.utils.data.DataLoader(
            image_dataset_val, batch_size=4, shuffle=True, num_workers=4
        )
    ...
    if args.train or args.finetune:
        ...
        if args.sam:
            ...
            optimizer_ft = SAM(
                model_ft.parameters(), base_optimizer, lr=0.001, momentum=0.9
            )

            if use_full_validation:
                model_ft = train_model_with_sam_and_full_val(
                    model_ft, criterion, optimizer_ft, num_epochs=10
                )
            else:
                model_ft = train_model_with_sam(
                    model_ft, criterion, optimizer_ft, num_epochs=10
                )
        else:
