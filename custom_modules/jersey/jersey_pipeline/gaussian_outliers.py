...
def get_main_subject(image_folder, feature_folder, threshold=3.5, rounds=3):
    ...
    for tr in tqdm(tracks):
        ...
        for r in range(rounds):
            ...
            cleaned_data = features[
                (euclidean_distance - mean_euclidean_distance) <= threshold
            ]
...
def get_main_subject_tracklab(features: np.ndarray, threshold=3.5, rounds=3):
    cleaned_data = features
    for r in range(rounds):
        # Fit a Gaussian distribution to the data
        mu = np.mean(cleaned_data, axis=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tracklets_folder", help="Folder containing tracklet directories with images"
    )
    parser.add_argument(
        "--output_folder", help="Folder to store features in, one file per tracklet"
    )
    parser.add_argument(
        "--threshold",
        type=float,
        default=3.5,
        required=False,
        help="Threshold for outlier removal per round, used to compute distance threshold*std",
    )
    parser.add_argument(
        "--rounds",
        type=int,
        default=3,
        required=False,
        help="Number of iteration for outlier removal",
    )
